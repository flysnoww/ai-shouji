...
- The local Admin Console operator was created by the user after Phase 1. No end-user account existed at Phase 1 completion. Phase 2 created one local test user through real Email OTP registration; no AIQuickNote data is present.
...
The local SMTP connector targets Mailpit without credentials; its required `auth` object is configured as `{"type":"login"}` and SMTP operation logging is disabled. The local Native/Public client is configured for Authorization Code + PKCE. Email verification code is enabled for sign-up and sign-in, and password sign-in remains disabled. Apple and Google remain unconfigured. A real Email OTP registration and PKCE ID-token verification succeeded; the local test subject is `gty8xvsquh6d` (non-secret OIDC `sub`). The existing `identity.ps1 smoke` also passed after restarting Logto and PostgreSQL, and this user row remained present. Logout followed by fresh OTP sign-in, including after restart, still requires end-to-end confirmation before this phase is considered ready. Do not expose production sign-up until duplicate-email social-registration behavior is checked against the pinned Logto version.
