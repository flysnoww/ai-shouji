...
- Simulator UI tests reproduced a second real hit-testing defect: ModuleList's plain row button exposes a full-row accessibility frame but its label only hit-tests text. Center-row taps never set selectedRecord, while Search's explicit contentShape works. Expand the list label and give it a rectangular contentShape. Also suppress link activation while a recognized visual drag is active; GestureState restores normal activation on release/cancellation.
