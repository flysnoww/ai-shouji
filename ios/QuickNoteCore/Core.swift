...
public struct DeterministicDraftParser: DraftParser {
    ...
    private func date(in text: String) -> Date? {
        ...
        if value == nil, text.range(of: #"(?:早上|上午|中午|下午|晚上)?\s*(?:\d{1,2}|[一二两三四五六七八九十]+)(?:点|:)|(?:at\s+)?\d{1,2}(?::\d{2})?\s*(?:AM|PM)|\bat\s+\d{1,2}(?::\d{2})?\b|\b(?:morning|afternoon|evening|noon|midnight)\b"#, options: [.regularExpression, .caseInsensitive]) != nil { value = calendar.startOfDay(for: now()) }
        ...
        else if let hourText = capture(in: text, pattern: #"\bat\s+(\d{1,2})(?::\d{2})?\b"#), let hour = Int(hourText), hour < 24 { let minute = Int(capture(in: text, pattern: #"\bat\s+\d{1,2}:(\d{2})\b"#) ?? "0") ?? 0; value = calendar.date(bySettingHour: hour, minute: minute, second: 0, of: value) ?? value }
    ...
    private func hasAction(in text: String) -> Bool { ["去", "买", "吃", "提交", "打电话", "充电", "看", "开会", "预约", "做", "拿", "送", "检查", "打扫", "提醒", "完成", "remind me", "call", "buy", "go", "send", "submit", "finish", "pick up", "check", "email", "water", "meet", "book", "take", "review", "pack", "appointment", "dentist"].contains { text.localizedCaseInsensitiveContains($0) } }
    ...
    private func dateRange(in text: String) -> (Date, Date)? {
        ...
        else if text.contains("今天") || text.contains("今晚") || lower.contains("today") || lower.contains("tonight") || lower.contains("this morning") || lower.contains("this afternoon") || lower.contains("this evening") { start = calendar.startOfDay(for: now()) }
