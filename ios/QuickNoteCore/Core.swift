...
public struct DeterministicDraftParser: DraftParser {
    ...
    private func hasFutureTime(in text: String) -> Bool { text.range(of: #"今天晚些时候|今晚|明天|后天|星期[一二三四五六日天]|周[一二三四五六日天]|下周|\d+\s*(?:小时|分钟)后|(?:早上|上午|中午|下午|晚上)?\s*(?:\d{1,2}|[一二两三四五六七八九十]+)(?:点|:)"#, options: .regularExpression) != nil }
