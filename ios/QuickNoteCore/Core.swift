...
public struct DeterministicDraftParser: DraftParser {
    ...
    private func draft(_ text: String) -> Record {
        ...
        if text.contains("提醒") || text.contains("待办") || (hasFutureTime(in: text) && (hasAction(in: text) || ["breakfast", "dentist", "appointment"].contains(where: { text.localizedCaseInsensitiveContains($0) }))) || text.contains("别忘了") {
    ...
    private func amountItems(in text: String) -> [AmountItem] {
        ...
        guard let regex = try? NSRegularExpression(pattern: #"(\d+(?:\.\d+)?)\s*(美元|美金|dollars?|USD|日元|JPY|円|yen|欧元|euros?|EUR|€|人民币|CNY|RMB|元|块|\$|¥|￥|£)"#, options: .caseInsensitive) else { return [] }
    ...
    private func currency(in text: String) -> String? {
        ...
        if text.contains("欧元") || text.contains("欧") || text.localizedCaseInsensitiveContains("EUR") || text.localizedCaseInsensitiveContains("euro") || text.contains("€") { return "EUR" }
    ...
    private func dateRange(in text: String) -> (Date, Date)? {
        ...
        guard let start else { return nil }; let component: Calendar.Component = weekday != nil ? .day : (text.contains("周") || lower.contains("week")) ? .weekOfYear : (text.contains("月") || lower.contains("month")) && !text.contains("明天") ? .month : .day; return (start, calendar.date(byAdding: component, value: 1, to: start)!)
    }
