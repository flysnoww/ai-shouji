...
@MainActor final class AppModel: ObservableObject {
    ...
    init(identityProvider: (any IdentityProvider)? = nil, store suppliedStore: RecordStore? = nil, defaults: UserDefaults = .standard) {
        ...
        if arguments.contains("--ui-regression-tests") || arguments.contains("--ui-identity-flow-tests") {
            let identityFlow = arguments.contains("--ui-identity-flow-tests")
            let testDefaults = UserDefaults(suiteName: "AIQuickNote.UIRegression")!
            let initialState: IdentityState = identityFlow ? .signedOut : .signedIn(FireseedUser(stableUserID: "ui-test-owner", email: nil, displayName: "UI Test"))
...
struct ModuleCard: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .overlay(RoundedRectangle(cornerRadius: 30).stroke(.white.opacity(0.95), lineWidth: 1.2).allowsHitTesting(false))
