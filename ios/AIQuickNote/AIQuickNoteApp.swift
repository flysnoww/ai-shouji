...
private let background = Color(uiColor: .systemGroupedBackground)
...
struct HomeView: View {
    ...
    var body: some View {
        ScrollView { LazyVGrid(columns: [.init(.flexible()), .init(.flexible())], spacing: 16) { ForEach(Module.allCases) { module in NavigationLink(value: module) { ModuleCard(module: module, records: model.records.filter { $0.module == module }) }.simultaneousGesture(TapGesture().onEnded { focused = false }) } }.padding() }
            .background(background.onTapGesture { focused = false }).safeAreaInset(edge: .bottom) { captureBar }.navigationTitle("AI 随手记")
...
struct ModuleListView: View {
    ...
    var body: some View { List(model.records.filter { $0.module == module }) { record in NavigationLink(value: record) { RecordRow(record: record) } }.scrollContentBackground(.hidden).background(background).navigationTitle(module.title).toolbar { Button { adding = true } label: { Image(systemName: "plus") } }.sheet(isPresented: $adding) { NavigationStack { ModuleComposerView(module: module) } }.navigationDestination(for: Record.self) { DetailView(record: $0) }.overlay { if model.records.allSatisfy({ $0.module != module }) { ContentUnavailableView("暂无记录", systemImage: module.icon) } }.onAppear { model.reload() } }
}
...
struct SearchView: View {
    ...
    var body: some View { List { Section("查询条件") { TextField("关键词", text: $keyword).focused($focused).onSubmit { focused = false; search() }; ScrollView(.horizontal) { HStack { ForEach(Module.allCases) { module in Toggle(module.title, isOn: binding(module)).toggleStyle(.button) } } }; Toggle("重要", isOn: $important) }; Section("结果 \(results.count)") { ForEach(results) { record in NavigationLink(value: record) { RecordRow(record: record) }.simultaneousGesture(TapGesture().onEnded { focused = false }) } } }.scrollContentBackground(.hidden).background(background.onTapGesture { focused = false }).navigationTitle("搜索").navigationDestination(for: Record.self) { DetailView(record: $0) }.onAppear { focused = false; search() }.onChange(of: modules) { search() }.onChange(of: important) { search() } }
...
struct RecordForm: View {
    ...
    private func save() { focused = false; var value = record; value.tags = tags.csv; if value.module == .ledger { guard amount.isEmpty || Decimal(string: amount) != nil else { model.error = "金额格式无效"; return }; value.amount = amount.isEmpty ? nil : Decimal(string: amount) }; record = value; onSave(value) }
}
