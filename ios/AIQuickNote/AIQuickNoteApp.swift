...
struct ModuleListView: View {
    ...
    var body: some View { List(model.records.filter { $0.module == module }) { record in NavigationLink(value: record) { RecordRow(record: record) } }.scrollContentBackground(.hidden).background(appBackground).navigationTitle(module.title).toolbar { Button { adding = true } label: { Image(systemName: "plus") } }.sheet(isPresented: $adding) { NavigationStack { ModuleComposerView(module: module) } }.navigationDestination(for: Record.self) { DetailView(record: $0) }.overlay { if model.records.allSatisfy({ $0.module != module }) { ContentUnavailableView("暂无记录", systemImage: module.icon) } }.onAppear { model.reload() } }
}
